...
func NotificationHandler(w http.ResponseWriter, r *http.Request, userID string) {
	...
	log.Printf("REDIRECT: Use WebSocket Manager at ws://websocket_manager:8003/ws/notifications/%s", userID)
	...
	w.Write([]byte(`{
		...
		"websocket_manager_url": "ws://websocket_manager:8003/ws/notifications/` + userID + `"
	}`))
} 